	...
	sunlight.registerLanguage("javascript", {
		...
		embeddedLanguages: {
			xml: {
				...
				//if true, this indicates to the parser to switch to XML mode
				switchTo: function(context) {
					var next5 = context.reader.peek(5);
					return context.reader.current() === "<" && (next5 === "html>");
				},

				//the "switchBack" function is called after the current character is processed
				//if true, this indicates to the parser to switch back to Javascript mode from XML
				switchBack: function(context) {
					var prev6 = context.reader.peek(-6);
					return context.reader.current() === ">" && (prev6 === "</html");
				}
			}
		}
	});
